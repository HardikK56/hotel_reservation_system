...
class Roombooking:
    def __init__(self,root):
        ...
        txtSubTotal.grid(row=7,column=1)
        ...
        #=============Right side image==========
        img3=Image.open(r"C:\Users\hardi\OneDrive\Desktop\DBMS_Prj\hotelimg5.jpg")
        ...
        details_table.place(x=0,y=50,width=830,height=180)
    ...
    def update(self):
        if self.var_contact.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="bhawnani@27012002",database="world")
            my_cursor=conn.cursor()
            my_cursor.execute("update room set check_in=%s,check_out=%s,roomtype=%s,Room=%s,meal=%s,noOfdays=%s where Contact=%s",(self.var_checkin.get(),
                self.var_checkout.get(),
                self.var_roomtype.get(),
                self.var_roomavailable.get(),
                self.var_meal.get(),
    ...
    #delete function
    def mDelete(self):
        mDelete=messagebox.askyesno("Hotel Management System","Do you want to delete this romm?",parent=self.root)
        if mDelete>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="bhawnani@27012002",database="world")
            my_cursor=conn.cursor()
